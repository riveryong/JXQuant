...
if __name__ == "__main__":
    try:
        mysqlet = MysqlHelper(host="127.0.0.1", user="root", password="root", charset="utf8", database="youke", port=3306)
        # 根据字段统计count, join>>AND,OR,可以不传，默认为AND
        print(mysqlet.findKeySql(const.COUNT, table="t_company", params={"company_id": "114"}, join="AND"))
    except Exception as e:
        print("执行sql出错:")
        raise e
        mysqlet.close()
